...
ontol_db:parent(X,domain,Y) :- property_domain(X,Y).
ontol_db:parent(X,range,Y) :- property_range(X,Y).

:- abolish(ontol_db:belongs/2).
ontol_db:belongs(X,class) :- class(X).
ontol_db:belongs(X,relation) :- property(X).
