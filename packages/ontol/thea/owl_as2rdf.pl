...
:- module(owl_as2rdf,
	  [ 	    	    
	    owl_generate_rdf/1, % RDF_Load_Mode (complete/not)
...
owl_generate_rdf(+FileName, +RDF_Load_Mode)
     ...
     first removed.   
*/
owl_generate_rdf(RDF_Load_Mode) :- 
	(   RDF_Load_Mode=complete,rdf_retractall(_,_,_); true),
        !,
	retractall(blanknode_gen(_,_)),
	owl_as2rdf_ontology, % CJM - moves this up; needs to be in header?
	owl_as2rdf_class,
	owl_as2rdf_subclass,
...
owl_as2rdf_property :- 
	...
	owl_as2rdf_triple_list(PID,_,AnnotationList),
        maplist(owl_as2rdf_subproperty(PID),PID_SuperList),
...
% CJM
owl_as2rdf_subproperty(PID,subObjectPropertyChain(SOPL)) :-
        !,
        owl_as2rdf(subObjectProperty(PID,subObjectPropertyChain(SOPL)), _BNode).


owl_as2rdf_subproperty(PID,PID_Super) :-
	owl_as2rdf_triple_list(PID,'rdfs:subPropertyOf',[PID_Super]).
...
_:x owl11: propertyChain T(SEQ op1 ... opn)
  */
owl_as2rdf(SubProp,Node) :- 
        SubProp = subObjectProperty(P,subObjectPropertyChain(SOPL)),
        !,
        owl_as2rdf_bnode(SubProp,Node),
	owl_as2rdf_list(SOPL,ListNode),
	%owl_as2rdf_bnode(propertyChain(PL),PropertyChainNode),
	owl_rdf_assert(Node,'rdfs:subPropertyOf',P),
	owl_rdf_assert(Node,'owl:propertyChain',ListNode).
