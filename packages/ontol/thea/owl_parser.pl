...
%% class(ClassURI,IsDeprecated,Complete,Annotations,Descriptions)
:- dynamic(class/5).
:- dynamic(owl/4).
:- dynamic(subclassOf/2).
%% property(PropertyId,Deprecated,Annotations,SuperPropertyList,PropertyTypeList,DomainList,RangeList)
:- dynamic(property/7).
...
%	(   OWL_Parse_Mode=complete,!,owl_clear_as; true),
	...
	owl_parser_log(['Number of unused remain triples ', Z11]).
...
use_owl(X1,Y1,Z1) :- 
	...
	expand_ns(Z1,Z),
	owl(X,Y,Z, not_used),
	debug(owl_parser_detail,'not_used ~w ~w ~w',[X,Y,Z]),
...
owl_parse_disjoint_classes :-
	findall(X-Y,use_owl(X,'owl:disjointWith',Y),Arcs),
        length(Arcs,NumArcs),
        debug(owl_parser,'owl_disjoint_arcs. Number of pairs = ~w',[NumArcs]),
        MaxArcs = 500,
        (   NumArcs > MaxArcs
        ->  format(user_error,'Too many disjoint pairs. ~w > ~w. Ignoring all',[NumArcs,MaxArcs])
...
owl_disjoint_arcs([A-B|T]) :-
        debug(owl_parser_detail,'owl_disjoint_arcs H=~w',[A-B]),
