...
/**
  ...
  @s1|Name| ontol_bridge_from_av_noun - masquerade av_db as an ontol_db

  @s1 Synopsis

  @cl
:- use_module(ontol_bridge_from_av_noun).
...
@cl
blip -r obol_av -u ontol_bridge_from_av_noun io-convert -to ontol_db:pro > ontol_av_noun.pro 
@/cl
...
:- module(ontol_bridge_from_av_noun, []).

:- use_module(av_db,[]).

ontol_db:class(N,N):- av_db:noun(N,_,_).
ontol_db:belongs(ID,O):- av_db:noun(ID,_,O).
