...
testax(X):-
        format('~nABSTRACT SYNTAX TERM: ~w~n',[X]),
        (   owl_as2prolog(X,[])
        ->  true
        ;   format('cannot write!~n')),
        assert(owl_parser:X),
        owl_generate_rdf(foo,_),
        format('== RDF ==~n'),
        shell('cat foo',_),
        format('=========~n'),
        shell('rm foo',_),
        retract(owl_parser:X),
        nl.

mytest:-
	testax(subclassOf(a,b)),
	testax(subclassOf(a,intersectionOf([b,c]))),
	testax(equivalentSet([a,intersectionOf([b,c])])),
	testax(subclassOf(intersectionOf([a,restriction(r,someValuesFrom(c))]),
                          intersectionOf([b,restriction(p,allValuesFrom(d))]))),
        nl.
...
*/
	owl_as2prolog(class(myclass,false,complete,_,
                            [intersectionOf([a,b,
                                             restriction(pid,
                                                         allValuesFrom(z))])]),
                      []),
	...
	owl_as2prolog(subclassOf(a,b),
                      []),

	owl_as2prolog(subclassOf(a,intersectionOf([b,c])),
                      []),

	owl_as2prolog(subclassOf(a,intersectionOf([b,c])),
                      []),

        owl_as2prolog(class('http://www.w3.org/2002/03owlt/miscellaneous/consistent001#WhiteWine',  
                            false, 
                            complete, 
                            [], 
