...
A Prolog system for processing Perl style regular expressions can be
...
This is a useful case study of symbolic computing in Prolog and also
an exploration of the semantics of regular expressions.
...
To represent regular expressions as symbolic objects in Prolog, we
specify that union/2, conc/2, star/1, plus/1, and group/1 represent
the five types of structured (recursively defined) regular
expression. The symbolic atoms any and eos represent the
metacharacters "." and "$", while char/1 represents a single
character. Positive and negative sets are represented respectively as
posSet/1 and negSet/1, in which set items are enclosed in a list. The
set items may be individual char/1 items or range/2 structures for
character ranges.  DCG Parser for Regular Expressions

Constructing the DCG parser requires apply the left recursion removal
techniques illustrated earlier for both the <RE> and <basic-RE>
productions. We also left factor the <simple-RE> production to avoid
backtracking.

---+ USE

To use the tokenizer for lexical analysis, we now need only define a
regular expression that specifies the allowable forms of tokens and
whitespace. Tokens should be included inside parenthesized regular
expressions so they are returned; whitespace should not be included in
parenthesized expressions. For example, if we consider the tokens for
the numeric expression grammar described previously, an appropriate
regular expression is " +|([0-9]+|\+|-)". Note that this expression
defines 4 alternative string types: (1) sequences of one or more
spaces (" +"), (2) sequences of one or more digits ("[0-9]+"), (3) the
+ operator (which must be escaped because it is a metacharacter), and
(4) the - operator. However, only the last three are selected as
tokens by inclusion within parentheses.

Finally, to use this expression in the Prolog tokenizer, the escape
character itself must be escaped due to Prolog's string syntax
conventions.


==
regmatch_atom(' +|([0-9]+|\\+|-)', '12 + 4 - 29', L).
L = [12,+,4,-,29].
==
...
One additional point worth considering is that you can force Prolog to
backtrack and undo decisions made by the "longest match" rule. The
following script illustrates what happens when each tokenization
produced is rejected by the user typing the ";" operator.
...
illustrate both the power of regular expressions and their semantics
in logical form. It is not intended to be a practical tool. However,
the built-in regular expression support provided by many scripting
languages (Perl, Javascript and so on), Unix tools (emacs, ex, grep,
and so on), and lexical analyzer generators (lex, flex, flex++, and so
on) are indeed practical and can greatly simplify string
processing. The use of regular expressions for lexical analysis is a
standard technique that is widely used in symbolic computing
applications.
...
The tokenize/3 predicate will do the whole job, given a satisfactory
regular expression!

*/

%% regmatch_atom(+RegExpAtom,+InAtom,?Matches) is nondet
% unifies Matches with list of tokens by repeated application of RegExpAtom to InAtom
regmatch_atom(RE,In,Matches):-
        atom_codes(RE,RE_Codes),
        atom_codes(In,In_Codes),
        tokenize(RE_Codes,In_Codes,Matches).
%        maplist(codes_atom,Matches,Matches2).

codes_atom(CL,A):- atom_codes(A,CL).
...
unittest(test(re,
            [],
            (   findall(P,regmatch_atom('(.)',
                                        abcfnorddeffnordxyzfnord,
                                        P),
                        Ps),
                writeln(ps=Ps),
                nl
...
unittest(test(html,
            [],
            (   findall(P,regmatch_atom('(.*)<i>([a-z]+)</i>(.*)',
                                        'abc <i>hij</i> klm <b>def</b>.ll;<i>s</i>al',
                                        P),
                        Ps),
                writeln(ps=Ps),
                nl
